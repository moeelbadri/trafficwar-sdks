...
class TrafficWar(BaseTrafficWar):
    """Reusable synchronous TrafficWar server-ingestion client."""
    ...
    def capture(self, event: Event, *, idempotency_key: Optional[str] = None) -> IngestResult:
        """Durably ingest one event."""

        return self._send(self._prepare_capture(event, idempotency_key))
    ...
    def capture_batch(
        ...
        *,
        ...
        return self._send(self._prepare_batch(events, idempotency_key))
